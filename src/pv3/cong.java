...
public class cong {
	...
	static final int lg = 5;// bien so toi thieu
	...
	public static List<point> creatp(path p) {
		...
		boolean tl;
//		point b = p.get(dau);
		int da, db, dx = 0, dy = 0, node = ls, stack = ls;
		boolean bres = false;
		while (a != dau) {
			da = Math.abs(p.get(a).getx() - p.get(ls).getx());
			db = Math.abs(p.get(a).gety() - p.get(ls).gety());
			dx = dx + da;
			dy = dy + db;
			if (dx * dy != 0) {
				if (dx > lg || dy > lg) {
			...
			if (bres) {
				...
				double[] sd = creline.region(p.get(stack), p.get(a));
				tl = change(res, sd);
				if (!tl) {
					ret.add(p.get(a));
					res[1] = -str;
